...
describe 'configuring cloud crooner' do
  context 'using settings from the environment' do
    context 'with default settings and no manifest' do
      ...
      it "creates a manifest " do
        expect app.manifest.to be_an_instance_of(Sinatra::Manifest)
        expect app.manifest.path.to match(/public\/static\/manifest-[\da-z]*\.json/)
        expect app.manifest.dir.to eq('public/static') 
      end
      ...
      it "sets the location of static assets to the parent directory of the manifest" do
        expect CloudCrooner.config.local_assets_dir.to eq('public/static')
      end
